function [MSE_sum,pred_sub] = MSE_ResErr(tau_est,JS,start_pos,exp_sub)
%% calculate believed residual error based on Tau estimate
% compute MSE of believed residual error from expected stop position (given by the bias).
...
for j = 1:length(JS.x)
    ...
    a = exp(-dt/tau_est(j));
    vmax = findvmax(x,T,tau_est(j));
    wmax = findvmax(ang,T,tau_est(j));
...
if 0
    if ~isempty(exp_sub)
     ...
     if ~isempty(exp_sub)
         ...
         MSE_sum = median( (PenaliseAngle*(pred_sub.x - exp_x).^2 + PenaliseDistance*(pred_sub.y - exp_y).^2) );
     else
         MSE_sum = [];
     end
end