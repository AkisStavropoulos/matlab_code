...
hold on; 
for i=1:length(x_fly2) 
    plot(x_fly2(i),y_fly2(i),'.r','markers',sqrt(round(100*(T - T_fly2(i))/T))); 
end
x = linspace(-R,R,1000); plot(x,sqrt(R^2 - x.^2),'k'); plot(x,-sqrt(R^2 - x.^2),'k'); % arena boundary
axis equal; axis([-1000 1000 -1000 1000]); axis off; 
...
subplot(2,1,2); 
imagesc(x,y,P_fly,[0 1e-3]);        % if you change N, make sure to change the upper limit accordingly
set(gca,'YDir','normal');
colormap('jet'); hold on;
x = linspace(-R,R,1000); plot(x,sqrt(R^2 - x.^2),'k'); plot(x,-sqrt(R^2 - x.^2),'k'); % arena boundary
axis equal; axis([-1000 1000 -1000 1000]); axis off; 
...
%    for j=1:Nbootstrap
%        indx = randperm(N^2);        
%        [r(j,i)] = nancor(P_monk_masked_rot_temp(indx(1:round(0.5*N^2))),P_fly_masked_temp(indx(1:round(0.5*N^2))));
%   end
% end
% r_mu = mean(r);
% r_std = std(r);
...
box off; title('Inter-reward interval'); ylabel('probability distribution of intervals'); xlabel('time (s)');
...
figure; plot(n,p/sum(p)); xlim([0 10]); title('distribution of interval between reward and last flash');...
...
%% distribution of errors (stopping position relative to the nearest target)
indx = find(v<5); indx2 = [];
for i=2:length(indx)
...
axis([-100 100 -100 100]); title('Distribution of stopping locations relative to the (nearest) target');
...
hold on; x = linspace(-R,R,1000); plot(x,sqrt(R^2 - x.^2),'k'); plot(x,-sqrt(R^2 - x.^2),'k'); % arena boundary
title('Vector field of errors'); % Vector field of errors (one vector for each stopping location). Vector heads are located at the stopping location and the tails at the nearest target.
%
sigma = 20; % width of gaussian filter for smoothing
...
figure; imagesc(P_stop); title('Probability distribution of stops within the arena');
%% distribution of egocentric target locations conditioned on angular acceleration
% unfiltered w
[peaks,peaklocs] = findpeaks(w,'MinPeakHeight',20,'MinPeakProminence',20);  % w: angular velocity degrees/s
t_peak = t_smr(peaklocs);
t_peak2 = t_peak(1); 
peaklocs2 = peaklocs(1);
for j=2:length(t_peak)
    if (t_peak(j) - t_peak2(end))>0.3
        t_peak2 = [t_peak2 t_peak(j)]; peaklocs2 = [peaklocs2 peaklocs(j)]; 
    end
end
...
filtered_w = conv(data,my_filter,'same');

[peaks,peaklocs] = findpeaks(filtered_w,'MinPeakHeight',20,'MinPeakProminence',20);  % w: angular velocity
t_peak = t_smr(peaklocs);
figure; plot(t_smr, filtered_w); hold on; vline(t_peak); title('Angular velocity peaks check');
%
w_peak = w(peaklocs);
w_ts = interp1(t_smr,w,ts);
theta = mod(cumsum(w)*dt,360); % integral of w
theta_ts = interp1(t_smr,theta,ts); 
theta_ts(1) = 0;
figure; hold on;
indx = 80000:100000;
...
for i=indx(1:50:end)
    quiver(x_monk2(i),y_monk2(i),20*sind(theta_ts(i)),-20*cosd(theta_ts(i)),0,'ok','markersize',2,... 
        'ShowArrowhead','on','markerFaceColor','k','MaxHeadSize',2); % (sinx,cosx) = tanx
end
hold on; x = linspace(-R,R,1000); plot(x,sqrt(R^2 - x.^2),'k'); plot(x,-sqrt(R^2 - x.^2),'k'); % arena boundary
%%
...
[X,Y] = meshgrid(x,y); 
P_mask = double((sqrt(X.^2 + Y.^2)<400) & (abs(atand(X./Y))<40)); % show 0 to 4 m & -40 to 40 deg, field of view
P_mask(P_mask==0) = NaN;
...
for k=1
    ...
    for i=1:length(t_peak)
        X_fly_temp = X_fly((ts>(t_peak(i)+t_pre) & ts<(t_peak(i)+t_post)),:);   % flies positions for this time period
        Y_fly_temp = Y_fly((ts>(t_peak(i)+t_pre) & ts<(t_peak(i)+t_post)),:);
        I_fly_temp = I_fly((ts>(t_peak(i)+t_pre) & ts<(t_peak(i)+t_post)),:);   % I_fly in 0/1 for on/off status (for this time period)
        indx_flyON = find(any(I_fly_temp));              % 33/200 are ON at that time period
        I_fly_ONtime = sum(I_fly_temp);            % time bins in that time period for which it was on
        I_fly_ONtime = I_fly_ONtime(indx_flyON);   % fireflies on for that period
        X_fly_temp = X_fly_temp(:,indx_flyON);     % position of ON fireflies
        Y_fly_temp = Y_fly_temp(:,indx_flyON);
        X_flyON = []; Y_flyON = [];
        for j=1:size(X_fly_temp,2)
            X_fly_temp2 = unique(X_fly_temp(X_fly_temp(:,j)~=0,j)); 
            X_flyON(j) = X_fly_temp2(1);
            Y_fly_temp2 = unique(Y_fly_temp(Y_fly_temp(:,j)~=0,j)); 
            Y_flyON(j) = Y_fly_temp2(1);
        end
        % compute field-of-view (fov)
        a = 500; % a=500 cm
        theta_peak(i) = theta_ts(find(ts>(t_peak(i)+t_pre),1));             % direction
        x_monk_peak(i) = x_monk(find(ts>(t_peak(i)+t_pre),1));          % monkey position at peak time
        y_monk_peak(i) = y_monk(find(ts>(t_peak(i)+t_pre),1));
        R = [cosd(-theta_peak(i)) -sind(-theta_peak(i)); sind(-theta_peak(i)) cosd(-theta_peak(i))]; % rotation matrices
        Rinv = [cosd(theta_peak(i)) -sind(theta_peak(i)); sind(theta_peak(i)) cosd(theta_peak(i))];
        XY_corners = [-a/2 a/2 -a/2 a/2; 0 0 a a];  % sides of the square on which the mask will be applied
        
        XY_corners2 = R*XY_corners; % rotate square in the direction of the monkey
        XY_corners2(2,:) = -XY_corners2(2,:); % flip y-coord
        XY_corners2(1,:) = XY_corners2(1,:) + x_monk_peak(i);   % shift to the position of the monkey
        XY_corners2(2,:) = XY_corners2(2,:) + y_monk_peak(i);
        % identify targets within fov
        indx = [];
        for j=1:length(X_flyON) % use projections to check whether a point is included in the square
            ...
            if (p12m>0 && p12m<p12) && (p13m>0 && p13m<p13)
                indx = [indx j]; % a point is included in the square if the projections of it fall on both the sides of the square
            end
        end
    ...
    P_fly_cw = P_fly_cw/sum(P_fly_cw(:)); 
    P_fly_ccw = P_fly_ccw/sum(P_fly_ccw(:));
    ...
    R_overlap(k) = nansum(nansum(P_fly_ccw0.*P_fly_cw0))/sqrt(nansum(nansum(P_fly_ccw0.^2))*nansum(nansum(P_fly_cw0.^2)));  % WHAT???
end
