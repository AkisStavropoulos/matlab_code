...
#
# =============================================================================
DIRECT = 'Z:\Data\Monkey2_newzdrive\Jimmy\\U-probe' # os.path.dirname(user_paths.get_path('cluster_data','m73s5'))
...
sv_folder = 'C:/Data/firefly_analysis/LFP_band/concatenation_with_accel/'
    ...
    for name in files:
        if re.match(pattern_fh,name):
            ...
            if not os.path.exists(os.path.join(sv_folder,svname)):
                ...
                fld_list += [root]
                print('.....adding session %s'%name.split('.')[0])
                
# # matchers = ['m73s8','m73s9','m73s10'] # ripple recordings
# matchers = ['m73s17','m73s18','m73s20','m73s21','m73s22']
# matching = [c for c in concat_list if any(xs in c for xs in matchers)]
# concat_list = matching
# # matchers = ['May 17 2021','May 20 2021','May 27 2021'] # ripple recordings
# matchers = ['Aug 04 2021','Aug 05 2021','Aug 18 2021','Aug 19 2021','Aug 30 2021']
# matching = [c for c in fld_list if any(xs in c for xs in matchers)]
# fld_list = matching
...
# fld_list = ['C:/Data/firefly_analysis/LFP_band/DATASET_accel/']*len(concat_list)
...
base_file = 'C:/Data/firefly_analysis/LFP_band/DATASET_accel/'
...
linearprobe_sampling_fq = 20000 # Plexon: 20000Hz, Ripple: 30000Hz
...
for session in concat_list:
    ...
    if session in ['m51s43','m51s38','m51s41','m51s42','m51s40']:
        fhLFP = 'C:/Data/firefly_analysis/LFP_band/DATASET_accel/lfps_%s.mat'%session
    else:
        fhLFP = ''
    ...
    exp_data.set_filters('all', True) # for specific condition, write specific condition value instead of True
    ...
    try:
        y, X, trial_idx = exp_data.concatenate_inputs(*var_names, t_start=t_start, t_stop=t_stop) # y=spike count, X=behv_data, trial_idx=trial indx, same length as spike count
    ...
    res['time_bin'] = 0.006 # sampling rate
    res['unit_info'] = {}
    ...
    if send:
        try:
            ...
            if 'eb162' in dest_folder:
                dest_folder=dest_folder.replace('eb162','ges6')
            ...
            os.system('sshpass -p "%s" scp %s ges6@greene.hpc.nyu.edu:%s' % ('Akissta555', os.path.join(sendfrom,'%s.npz'%session),dest_folder))
        except Exception as e:
            print(e)
